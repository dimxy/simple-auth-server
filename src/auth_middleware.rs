    ...
    B: 'static,
{
    ...
    fn call(&self, request: ServiceRequest) -> Self::Future {
        ...
        if !is_logged_in && !["/api/auth", "/api/invitation", "/api/register"].iter().any(|unprotected| request.path().starts_with(unprotected)) {
            let (request, _pl) = request.into_parts();

            let response = HttpResponse::Found()
                .insert_header((http::header::LOCATION, "/api/auth"))
                .finish()
